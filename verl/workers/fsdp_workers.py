...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_ref:
            if self.config.ref.get("model.path", None) is None:
                local_path = copy_to_local(self.config.model.path, use_shm=use_shm)
            else:
                print("## Using ref model path from config.ref.model.path")
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            print("## Preparing ref data for compute_log_prob")
            ref_data = copy.deepcopy(data)
            ref_prompt_length = ref_data.batch["ref_input_ids"].shape[1]
            ref_data.batch["prompts"][:, :ref_prompt_length] = ref_data.batch["ref_input_ids"][:, :ref_prompt_length]
            ref_data.batch["input_ids"][:, :ref_prompt_length] = ref_data.batch["ref_input_ids"][:, :ref_prompt_length]
            ref_data.batch["attention_mask"][:, :ref_prompt_length] = ref_data.batch["ref_attention_mask"][:, :ref_prompt_length]
            bsz = ref_data.batch["responses"].shape[0]
            response_length = ref_data.batch["responses"].shape[1]
            delta_position_id = torch.arange(1, response_length + 1, device=ref_data.batch["input_ids"].device)
            delta_position_id = delta_position_id.unsqueeze(0).repeat(bsz, 1)
            ref_response_position_ids = ref_data.batch["ref_position_ids"][:, -1:] + delta_position_id
            ref_position_ids = torch.cat([ref_data.batch["ref_position_ids"], ref_response_position_ids], dim=-1)
            ref_data.batch["position_ids"] = ref_position_ids

            output, _ = self.ref_policy.compute_log_prob(data=ref_data, calculate_entropy=False)
