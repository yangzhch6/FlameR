...
def compute_format_score(model_output: str, check_think_format: bool, check_step_format: bool):
    if check_think_format:
        if model_output.count("</think>") != 1:
            return -0.5

    if check_step_format:
        # "<step_bed619fva643c0v108hd53gcy>", "</step_bed619fva643c0v108hd53gcy>"
        step_open_tag = "<step_bed619fva643c0v108hd53gcy>"
        step_close_tag = "</step_bed619fva643c0v108hd53gcy>"
        try:
            assert model_output.count(step_open_tag) == model_output.count(step_close_tag)
            assert model_output.count(step_open_tag) > 0
        except:
            return -0.5
...
class MathVerifyRewardManager:
    ...
    def __init__(
        ...
        check_step_format: bool = False, # ["<step_bed619fva643c0v108hd53gcy>", "</step_bed619fva643c0v108hd53gcy>"],
        reward_fn_key: str = "data_source",
    ) -> None:
        self.tokenizer = tokenizer
        self.num_examine = num_examine  # the number of batches of decoded responses to print to the console
        ...
        self.check_step_format = check_step_format
    ...
    def verify(self, data):
        ...
        assert len(sequences_str) == len(ground_truth) == len(data_sources)
        self.compute_score(data_sources[0], sequences_str[0], ground_truth[0], extra_info[0])
        print("## sucess test single sample...")
        try:
            scores = run_reward_scoring(
                self.compute_score,
                completions=sequences_str,
        ...
        for i in range(len(sequences_str)):
            format_score = compute_format_score(
                sequences_str[i],
                check_think_format=self.check_think_format,
                check_step_format=self.check_step_format,
            )
        ...
        for i in range(len(scores)):
            final_score = scores[i] + format_scores[i]
            final_scores.append(final_score)
        final_scores = [max(0.0, min(1.0, s)) for s in final_scores]  # clamp to [0, 1]

        data.batch["acc"] = torch.tensor(final_scores, dtype=torch.float32, device=prompt_ids.device)
        return final_scores
