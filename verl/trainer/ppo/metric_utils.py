...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    math_metrics = {}
    if "math_verify_acc" in batch.batch:
        # 1. 获取预测值和真实标签
        rm_scores = batch.batch["acc"]
        ground_truth = batch.batch["math_verify_acc"]

        # 2. 二值化预测结果（其实本来就二值化了）
        predictions = (rm_scores > 0.5).float()
        labels = (ground_truth > 0.5).float()

        # 3. 计算 TP (True Positive), FP (False Positive), FN (False Negative)
        # TP: 模型预测为1，真实为1
        ...
        # 4. 计算 Precision 和 Recall (加 epsilon 防止除以 0)
        epsilon = 1e-8
        rm_precision = tp / (tp + fp + epsilon)
        rm_recall = tp / (tp + fn + epsilon)
        
        # 5. 计算 RM 的分类准确率 (Accuracy)
        rm_accuracy = (predictions == labels).float().mean()

        # 6. 将结果存入临时字典
        math_metrics = {
            "critic/rm_accuracy": rm_accuracy.detach().item(),
            "critic/rm_precision": rm_precision.detach().item(),
            "critic/rm_recall": rm_recall.detach().item(),
            # 同时也记录一下真实的 math pass rate (ground truth 的均值)
            "critic/math_verify_acc/mean": torch.mean(ground_truth.float()).detach().item(),
        }
